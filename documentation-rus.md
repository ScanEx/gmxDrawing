...
rotate | `<Bool>` | `false` | Флаг разрещающий поворот объекта(При значении `true` - поворот осуществляется вокруг центра отображаемого bounds объекта).
maxPoints | `<Number>` | 0 | При добавлении новых точек в режиме редактирования геометрии - ограничивает количество точек в геометрии.(При значении `0` количество точек не контролируется).
disableAddPoints | `<Bool>` | `false` | Флаг запрещающий добавление/удаление точек.
